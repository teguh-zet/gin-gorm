...
func NewLoanServer(router *gin.RouterGroup, db *gorm.DB, nc *nats.Conn, version string) *LoanServer {
	return &LoanServer{router: router, db: db, nc: nc, version: version}
}
...
func (s *LoanServer) Init() {
	...
	service := NewLoanService(s.db, s.nc)
